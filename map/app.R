# In order to become acquainted with plotly and maps in R, we explored data by state that indicates 
# the number of donations and the value of the donation made from people associated with each major 
#political party from all accross the country, to candidates in the state of Massachusetts. Our final 
#product is three maps, one for each major party, which displays the number of contributions made from 
#each state, and the value of these contributions. Two sliding bars have been added so that users can 
#scale the value and number of donators, making it easier to explore the data. After the bars have been
#adjusted, the map will show the number of users and values up to and including the number input by the
#user. The information will be indicated through the coloring of the map. 
library(leaflet)
library(sp)
library(magrittr)
library(maps)
  ...
  server = shinyServer(function(input, output) { 
    ...
    output$plot2 <- renderPlotly({
      ...
      p<- ggplot(data = data2()) + 
        ...
        scale_fill_gradient("Donations", low="cyan3", 
                            high="blue3", 
                            breaks=c(3.4, 4.7, 6),
                            labels=c("low", "", "high") ) +
    ...
    output$plot3 <- renderPlotly({
      ...
      p<- ggplot(data = data3()) + 
        geom_polygon(aes(x = long, y = lat, fill = Donations, 
                         group = group), color = "white", show.legend = T) + 
        coord_fixed(1.3) + 
        
        labs(title = "Donors to Massachusetts Independents",
             caption = "Number of Donors by State") +
        
        
        scale_fill_gradient("Donations", low="darkorchid1", 
                            high="darkorchid4", 
                            breaks=c(3.4, 4.7, 6),
                            labels=c("low", "", "high") ) +
